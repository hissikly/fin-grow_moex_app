...
function updateBalanceDisplay() {
  const profile = document.getElementById('profile-balance-value');
  if (profile) profile.textContent = userBalance;
  const deposits = document.getElementById('deposits-balance-value');
  if (deposits) deposits.textContent = userBalance;
  const game = document.getElementById('game-balance-value');
  if (game) game.textContent = userBalance;
}
...
function startInvestmentFlow() {
		...
		document.querySelector(`.game-level[data-level="7"]`).classList.add('active');
...
function initLevelDoneHandlers() {
  ...
  document.querySelectorAll('.game-level.available:not(.done)').forEach(level => {
    level.addEventListener('click', function(e) {
      ...
      if (levelNum === '7') {
        // Перенаправляем на специальный путь, который обработает сервер
    ...
    if (currentQuestionIndex < testQuestions.length - 1) {
      ...
      // Разблокируем следующий уровень (7-й)
			...
			if (nextLevel) {
					...
					initLevelDoneHandlers(); // Переинициализация обработчиков
			}
...
function showFinuslugiPracticeModal() {
  let modal = document.createElement('div');
  modal.className = 'finuslugi-modal';
  modal.innerHTML = `
    <div class="finuslugi-modal-content">
      <span class="finuslugi-modal-close" title="Закрыть">&times;</span>
      <h2>Интерфейс финуслуг для практики</h2>
      <p style="margin-top:16px;">Здесь будет интерфейс финуслуг для практики.</p>
    </div>
  `;
  document.body.appendChild(modal);
  modal.querySelector('.finuslugi-modal-close').onclick = () => modal.remove();
  modal.onclick = e => { if (e.target === modal) modal.remove(); };
}

function initLevel7ModalHandler() {
  const level7 = document.querySelector('.game-level[data-level="7"]');
  if (level7) {
    level7.style.cursor = 'pointer';
    level7.addEventListener('click', showFinuslugiPracticeModal);
  }
}
document.addEventListener('DOMContentLoaded', () => {
  updateBalanceDisplay();
